...
THINLTO_CACHE_DIR := --thinlto-cache-dir=.thinlto-cache
THINLTO_CACHE_POLICY := --thinlto-cache-policy=cache_size_bytes=5g:cache_size=0%
LDFLAGS		+= $(THINLTO_CACHE_DIR) $(THINLTO_CACHE_POLICY)
lto-clang-flags += -Wl$(comma)$(THINLTO_CACHE_DIR) -Wl$(comma)$(THINLTO_CACHE_POLICY)
else
